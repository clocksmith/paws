 ...
 */
function formatBlueprintInfo(blueprint: Blueprint): string {
 ...
 */
async function getCheckpointHistory(): Promise<Checkpoint[]> {
...
async function runSelfTests(): Promise<TestResult> {
  ...
  const mockTests = [
    { name: "VFS integrity", status: "pass" as const, duration_ms: 120 },
    { name: "DI container", status: "pass" as const, duration_ms: 85 },
    { name: "Blueprint loading", status: "pass" as const, duration_ms: 95 },
    { name: "Tool execution", status: "pass" as const, duration_ms: 150 },
    {
...
server.resource(
  "reploid://vfs/tree",
  "Virtual filesystem tree showing all REPLOID modules and files",
  async () => {
    ...
    // Group by directory
    const dirs = new Map<string, VFSEntry[]>();
    ...
    const topLevel = entries.filter(
      (e) => !e.path.includes("/") && e.type === "directory"
    );

    for (const dir of topLevel) {
      lines.push(`\n### ${dir.path}/`);
      const files = entries.filter(
        (e) => e.path.startsWith(dir.path + "/") && e.type === "file"
      );
 ...
 */
server.resource(
  "reploid://vfs/file/{path}",
  new ResourceTemplate("reploid://vfs/file/{path}", {
 ...
 */
server.resource(
  "reploid://blueprints/library",
  "Complete library of REPLOID architectural blueprints for self-improvement",
  async () => {
    ...
    // Group by category
    const byCategory = new Map<string, Blueprint[]>();
    for (const bp of blueprints) {
      if (!byCategory.has(bp.category)) byCategory.set(bp.category, []);
 ...
 */
server.resource(
  "reploid://blueprints/{id}",
  new ResourceTemplate("reploid://blueprints/{id}", {
    list: undefined,  // Dynamic resources don't need listing
  }),
  async (uri: URL, variables: Variables) => {
...
server.resource(
  ...
  "Git checkpoint history for REPLOID modifications",
  async () => {
 ...
 */
server.resource(
  "reploid://tests/latest",
  "Latest REPLOID self-test execution results",
  async () => {
...
server.tool(
  ...
  "Propose a modification to REPLOID's source code (REQUIRES APPROVAL)",
  {
    operation: z
  ...
  async ({ operation, file_path, new_content, rationale }) => {
    ...
    let old_content: string | undefined;
    try {
    ...
    lines.push(
      "5. Self-tests will validate (auto-rollback if <80% pass rate)"
    );
...
server.tool(
  "run_tests",
  "Execute REPLOID's self-test suite to validate system integrity",
  {},
  async () => {
    const results = await runSelfTests();
...
server.tool(
  "create_checkpoint",
  "Create a Git checkpoint to enable rollback if needed",
  {
    message: z
      .string()
...
server.tool(
  ...
  "Restore REPLOID to a previous checkpoint (DISCARDS CHANGES)",
  {
    checkpoint_id: z
      .string()
  ...
  async ({ checkpoint_id }) => {
    ...
    if (results.success_rate >= 0.8) {
      ...
      lines.push(
        "⚠️  **WARNING** - System degraded even after rollback, investigation needed"
      );
    }
...
server.tool(
  "query_blueprints",
  "Search REPLOID's architectural blueprints for implementation guidance",
  {
    query: z
      .string()
  ...
  async ({ query, category }) => {
    ...
    let matches = blueprints.filter(
      (bp) =>
        bp.title.toLowerCase().includes(queryLower) ||
    ...
    if (category) {
      matches = matches.filter((bp) =>
        bp.category.toLowerCase().includes(category.toLowerCase())
      );
    }
...
server.tool(
  ...
  "Analyze module dependencies to understand impact of changes",
  {
    file_path: z
      .string()
...
server.prompt(
  ...
  "Interactive session for REPLOID self-improvement cycle",
  {},
  () => ({
    messages: [
      {
...
server.prompt(
  ...
  "Apply an architectural blueprint to implement a new capability",
  {},
  () => ({
    messages: [
      {
...
server.prompt(
  ...
  "Experiment with changes safely using checkpoints and rollback",
  {},
  () => ({
    messages: [
      {
