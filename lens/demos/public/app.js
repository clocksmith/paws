  ...
  if (serverName === 'filesystem') {
    switch (toolName) {
      ...
      case 'get_file_info':
        return { path: './sandbox/example.txt' };
      case 'write_file':
        return { path: './sandbox/output.txt', content: 'Generated by MOP demo at ' + new Date().toISOString() };
      case 'search_files':
  ...
  if (serverName === 'sequential-thinking') {
    switch (toolName) {
      case 'sequentialthinking':
        return {
          thought: 'Analyzing the MOP demo dashboard functionality',
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.result?.content) {
        ...
        if (preview) {
          addLog('info', `→ ${preview}${preview.length >= 150 ? '...' : ''}`);
        }
      }
