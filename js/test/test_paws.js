  ...
  return new Promise((resolve, reject) => {
    exec(command, (error, stdout, stderr) => {
      ...
      if (error && error.code > 1) {
        return reject(
          new Error(`Command failed with code ${error.code}:\n${stderr}`)
        );
      }
...
describe("PAWS for Node.js", function () {
  ...
  describe("cats.js", () => {
    it("CLI should bundle files using glob patterns and respect excludes", async () => {
      const command = `node ${catsCliPath} "${path.join(
        tempDir,
        "src/**/*.js"
    ...
    it("CLI should prepend persona and sysprompt files in the correct order", async () => {
      ...
      const command = `node ${catsCliPath} "${path.join(
        tempDir,
        "README.md"
      )}" -p "${path.join(tempDir, "persona.md")}" -s "${path.join(
        tempDir,
        "system.md"
      )}" -o - -q`;
      const { stdout } = await runCli(command);
      ...
      expect(stdout.indexOf("PERSONA_CONTENT")).to.be.lessThan(
        stdout.indexOf("SYSTEM_PROMPT_CONTENT")
      );
      expect(stdout.indexOf("SYSTEM_PROMPT_CONTENT")).to.be.lessThan(
        stdout.indexOf("README.md")
      );
    });
...
🐕 --- DOGS_END_FILE: new/nested/dir/test.js ---
      ...
      const extractedContent = await fs.readFile(
        path.join(outputPath, "new/nested/dir/test.js"),
        "utf-8"
      );
...
Let me know if you need anything else!
      ...
      expect(extractedFiles[0].contentBytes.toString().trim()).to.equal(
        "const app = {}; // Main app object"
      );
    });
