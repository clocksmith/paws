...
class BenchmarkMetrics {
  constructor({
    ...
    this.solutionQuality = solutionQuality;  // 0.0 to 1.0
...
class PerformanceBenchmark {
  constructor(outputDir = 'workspace/benchmarks') {
   ...
   */
  estimateCost(modelId, tokenCount) {
   ...
   */
  async runBenchmarkSuite(suite, competitors) {
   ...
   */
  async generateReport(results, outputFile = 'benchmark_report.json') {
    const report = {
    ...
    for (const [task, metrics] of Object.entries(results)) {
      report.detailed_results[task] = metrics.map(m => ({
  ...
  _printReport(report) {
    ...
    for (const [modelName, stats] of Object.entries(summary)) {
      console.log(
        `${modelName.padEnd(25)} ` +
        `${(stats.pass_rate * 100).toFixed(1).padStart(6)}%     ` +
        `${stats.avg_execution_time.toFixed(1).padStart(6)}s      ` +
        `${Math.floor(stats.avg_token_count).toString().padStart(6)}      ` +
        `$${stats.total_cost.toFixed(3).padStart(6)}`
      );
    }
