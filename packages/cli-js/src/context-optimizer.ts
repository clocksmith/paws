#!/usr/bin/env node
/**
 * PAWS Context Optimizer - Handle 500K+ line refactors (TypeScript)
 *
 * Strategies:
...
class CodeModule {
  public path: string;
  public sizeLines: number;
  public imports: Set<string>;
  public exports: Set<string>;
  public complexityScore: number;
  public summary: string | null;

  constructor(filePath: string, sizeLines: number, imports: Set<string>, exports: Set<string>, complexityScore: number, summary: string | null = null) {
    this.path = filePath;
...
class ContextWindow {
  ...
  public summaryFiles: string[];
  public totalLines: number;
  public estimatedTokens: number;

  constructor(coreFiles: string[], summaryFiles: string[], totalLines: number, estimatedTokens: number) {
...
class DependencyAnalyzer {
  ...
  public modules: Map<string, CodeModule>;
   ...
   */
  async analyzeJavaScriptFile(filePath: string): Promise<CodeModule> {
    try {
   ...
   */
  async buildDependencyGraph(files: string[]): Promise<Map<string, Set<string>>> {
   ...
   */
  findDependencies(filePath: string, graph: Map<string, Set<string>>, maxDepth: number = 2): Set<string> {
    const dependencies = new Set<string>();
    const toVisit: Array<[string, number]> = [[filePath, 0]];
...
class ContextOptimizer {
  ...
  constructor(rootPath: string, maxTokens: number = 100000) {
    this.rootPath = rootPath;
   ...
   */
  generateCatscanSummary(filePath: string): string | null {
   ...
   */
  async createContextWindow(task: string, relevantFiles: string[]): Promise<ContextWindow> {
   ...
   */
  _rankFilesByRelevance(task: string, files: string[], graph: Map<string, Set<string>>): string[] {
   ...
   */
  async createOptimizedBundle(window: ContextWindow, outputPath: string): Promise<void> {
    const lines = [];
