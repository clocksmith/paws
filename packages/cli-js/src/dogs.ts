#!/usr/bin/env node
/**
 * Enhanced DOGS extractor with interactive review and verification features (TypeScript).
...
type FileOperationType = typeof FileOperation[keyof typeof FileOperation];
type FileStatus = 'pending' | 'accepted' | 'rejected' | 'skipped';
...
class FileChange {
  ...
  public newContent: string | null;
  public isBinary: boolean;
  public status: FileStatus;

  constructor(filePath: string, operation: FileOperationType, oldContent: string | null = null, newContent: string | null = null, isBinary: boolean = false) {
...
class ChangeSet {
  ...
  getRejected(): FileChange[] {
    return this.changes.filter(c => c.status === 'rejected');
  }

  getSummary(): { total: number; accepted: number; rejected: number; pending: number } {
    return {
...
class InteractiveReviewer {
   ...
   */
  async reviewWithBlessed(): Promise<ChangeSet> {
...
class GitVerificationHandler {
  ...
  async runVerification(command: string): Promise<{ success: boolean; output: string }> {
    ...
    return new Promise((resolve) => {
      ...
      execFile(cmd, args, { cwd: this.repoPath }, (error: any, stdout: any, stderr: any) => {
        if (error) {
...
interface BundleProcessorConfig {
  outputDir?: string;
  verify?: string;
  dryRun?: boolean;
  interactive?: boolean;
...
class BundleProcessor {
  private config: BundleProcessorConfig;
  private changeSet: ChangeSet;
  private gitHandler: GitVerificationHandler | null;
  private progressBus: ProgressBus | null;

  constructor(config: BundleProcessorConfig) {
    this.config = config;
   ...
   */
  async parseBundle(bundleContent: string): Promise<ChangeSet> {
    ...
    for (const line of lines) {
      ...
      if (match) {
        if (match[1].toUpperCase() === 'START') {
          ...
          currentContent = [];
        } else if (match[1].toUpperCase() === 'END' && inFile && currentFile) {
   ...
   */
  async processFile(filePath: string, contentLines: string[], isBinary: boolean): Promise<void> {
    // Clean up content
   ...
   * Clean content lines
   */
  cleanContent(lines: string[]): string[] {
   ...
   */
  async applyChanges(changeSet: ChangeSet): Promise<boolean> {
    let successCount = 0;
    let errorCount = 0;
      ...
      try {
        ...
        if (change.operation === FileOperation.DELETE) {
          ...
          if (change.newContent !== null) {
            if (change.isBinary) {
              await fs.writeFile(absPath, Buffer.from(change.newContent, 'base64'));
            } else {
   ...
   */
  async runWithVerification(changeSet: ChangeSet, verifyCommand: string): Promise<boolean> {
...
async function main(): Promise<number> {
  ...
  try {
    ...
    if (bundleFile === '-') {
      bundleContent = await new Promise<string>((resolve) => {
        let data = '';
...
async function extractBundle(options: any): Promise<any[]> {
 ...
 */
function applyDeltaCommands(originalContent: string, deltaContent: string): string {
  ...
  commands.sort((a, b) => {
    const lineA = ('line' in a) ? a.line : ('start' in a) ? a.start : 0;
    const lineB = ('line' in b) ? b.line : ('start' in b) ? b.start : 0;
    return lineB - lineA;
  });
