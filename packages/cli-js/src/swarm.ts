 ...
 */
const AgentRole = {
  ARCHITECT: 'architect' as const,      // High-level design decisions
  IMPLEMENTER: 'implementer' as const,  // Code implementation
  REVIEWER: 'reviewer' as const,        // Code review and critique
  TESTER: 'tester' as const            // Test case generation
};

type AgentRoleType = typeof AgentRole[keyof typeof AgentRole];
...
class SwarmAgent {
  public name: string;
  public role: AgentRoleType;
  public config: any;
  private _client: any;

  constructor(name: string, role: AgentRoleType, config: any) {
    this.name = name;
    this.role = role;
    this.config = config;
    this._client = null;
...
class SwarmMessage {
  ...
  public toAgent: string | null;
  public roundNum: number;
  public content: string;
  public messageType: string;

  constructor(fromAgent: string, toAgent: string | null, roundNum: number, content: string, messageType: string) {
...
class TaskDecomposition {
  ...
  public subtasks: TaskDecomposition[];
  public assignedTo: string | null;
  public status: string;
  public solution: string | null;

  constructor(taskId: string, description: string, subtasks: TaskDecomposition[] = [], assignedTo: string | null = null, status: string = 'pending', solution: string | null = null) {
    this.taskId = taskId;
...
class SwarmOrchestrator {
  ...
  public taskTree: TaskDecomposition | null;

  constructor(task: string, contextBundle: string, outputDir: string = 'workspace/swarm') {
    this.task = task;
  ...
  getAgentsByRole(role: AgentRoleType): SwarmAgent[] {
   ...
   */
  async decomposeTask(): Promise<TaskDecomposition> {
...
CONTEXT:
${this.contextContent ? this.contextContent.substring(0, 2000) : '(No context available)'}  // Truncated for brevity
...

{
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        const taskTree = new TaskDecomposition(
          'root',
          this.task,
          subtasks.map((st: any) => new TaskDecomposition(st.id, st.description))
        );
   ...
   */
  async solveSubtaskCollaboratively(subtask: TaskDecomposition, roundLimit: number = 3): Promise<string> {
  ...
  async _generateInitialSolution(implementer: SwarmAgent, subtask: TaskDecomposition): Promise<string> {
...
🐕 --- DOGS_END_FILE: path/to/file.js ---`;
  ...
  async _generateCritique(reviewer: SwarmAgent, subtask: TaskDecomposition, solution: string): Promise<string> {
...
Otherwise, provide specific, actionable feedback for improvement.`;
  ...
  async _reviseSolution(implementer: SwarmAgent, subtask: TaskDecomposition, originalSolution: string, critique: string): Promise<string> {
...
Address the reviewer's feedback and provide an improved solution using the same DOGS format.`;
   ...
   */
  async mergeSolutions(solutions: Record<string, string>): Promise<string> {
