...
while [ $iteration -le $MAX_ITERATIONS ]; do
    ...
    # Clear VFS cache on first iteration to ensure fresh start
    if [ $iteration -eq 1 ]; then
        echo "🗑️  Clearing VFS cache for fresh start..."
        if node integrations/claude-code/hooks/browser-automation.mjs clear-cache 2>&1; then
            echo "✅ Cache cleared"
        else
    ...
    if node integrations/claude-code/hooks/browser-automation.mjs awaken "$GOAL" 2>&1 | tee -a "$ERROR_LOG"; then
        echo ""
